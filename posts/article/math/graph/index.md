...
- 辺$e$が$G$の切断辺$\rightleftarrow e$を境界に持つ面は唯一

- オイラーの公式：頂点数を$n$、辺数を$m$、面数を$f$、連結成分の数を$k$としたとき
  $$n-m+f=1+k$$

- $n \geq 3$の時、$m \leq 3n - 6$

- $G$の双対グラフ$G*$は平面グラフ

- Kuratowskiの定理：$G$は$K_5,K_{3,3}$をマイナーに含まない
...
- 一般化ケイリーの公式
  - $n$頂点、$k$個の連結成分からなり、頂点$1$から頂点$k$が異なる連結成分に属するようなラベル付き森の個数は$k n^{n-k-1}$

- 次数制限付きケイリーの公式
  - 次数列が$d_1,d_2,\ldots,d_n(\sum_k d_k=2n-2)$で表される$n$頂点のラベル付き木の個数は
